...
const groupSchema = yup.object({
  ...
  webhookUrls: yup
    ...
    .min(0, "At least one webhook URL is recommended"),
});
...
export default function GroupForm({ groupId }: GroupFormProps) {
  ...
  const { data: groupData, isLoading: loading } = useApiQuery<{ group: Group }>(
    `/api/groups/${groupId}`,
    {
  ...
  const saveGroup = useApiMutation<Group, GroupFormData>({
    url: isEditMode ? `/api/groups/${groupId}` : "/api/groups",
  ...
  // Mutation for delete
  const deleteGroup = useApiMutation<void, void>({
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div>
          ...
          {errors.color && (
            <p className="text-sm text-red-400 mt-1">{errors.color.message}</p>
          )}
        </div>
